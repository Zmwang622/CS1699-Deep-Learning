...
FLAGS = flags.FLAGS # flags are used on the CLI 
...
"""
domain is the four different mediums we are trying to classify: 
art_painting, cartoon, photo, sketch

Category indicates the 7 different shapes we need to classify:
...
class pacs_dataset(Dataset):
  ...
  def __init__(self,
               ...
               is_training=False,
               transform=None):
    self.root_dir = os.path.join(root_dir, 'train' if is_training else 'val') # training use train directory
    self.label_type = label_type # label type is either domain or category
    self.is_training = is_training # training mode or eval mode
    if transform:
...
class AlexNetTiny(nn.Module):
  ...
  def __init__(self, configs):
    ...
    self.classifier = nn.Sequential(
      ...
      nn.Linear(2048,1024),
      nn.ReLU(inplace = True),
      nn.Linear(1024, configs['num_classes']),   
    )
...
class AlexNetDilation(nn.Module):
  ...
  def __init__(self, configs):
    ...
    self.configs = configs
    self.features = nn.Sequential(
      nn.Conv2d(3,96, kernel_size = 11, padding = 5, stride = 4, dilation = 2),
      nn.ReLU(inplace = True),
      nn.MaxPool2d(kernel_size = 3, stride = 2),
      nn.Conv2d(96,256, kernel_size = 5, padding = 4, dilation = 2),
      nn.ReLU(inplace = True),
      nn.MaxPool2d(kernel_size = 3, stride = 2),
      nn.Conv2d(256,384,kernel_size = 3, padding = 2, dilation = 2),
      nn.ReLU(inplace = True),
      nn.Conv2d(384,384,kernel_size = 3, padding = 2, dilation = 2),
      nn.ReLU(inplace = True),
      nn.Conv2d(384,256,kernel_size = 3, padding = 2, dilation = 2),
      nn.ReLU(inplace = True),
      nn.MaxPool2d(kernel_size = 3, stride = 2),
    )
    self.classifier = nn.Sequential(
...
def model_training():
  ...
  val_dataset = pacs_dataset(root_dir='pacs_dataset',
                            label_type=FLAGS.label_type,
                            is_training=False)
